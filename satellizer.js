...
(function(window, angular, undefined) {
    ...
    var storage = {
        get: function(tokenStorage, namespace) {

            var token;

            switch (tokenStorage) {
                case 'localStorage':
                    token = window.localStorage.getItem(namespace);
                    break;
                case 'cookie':
                    token = document.cookie.replace(new RegExp("(?:(?:^|.*;)\\s*" + encodeURIComponent(namespace).replace(/[\-\.\+\*]/g, "\\$&") + "\\s*\\=\\s*([^;]*).*$)|^.*$"), "$1");
        ...
        set: function(tokenStorage, namespace, token, options) {
            ...
            switch (tokenStorage) {
                case 'localStorage':
                    ...
                    break;
                case 'cookie':
                    if (!namespace || /^(?:expires|max\-age|path|domain|secure)$/i.test(namespace)) {
                        return false;
                    }
                    var expires = "",
                    ...
                    document.cookie = encodeURIComponent(namespace) + "=" + encodeURIComponent(token)
                        + expires
        ...
        delete: function(tokenStorage, namespace, options) {
            switch (tokenStorage) {
                case 'localStorage':
                ...
                case 'cookie':
                    ...
                    document.cookie = encodeURIComponent(namespace) + "=; expires=Thu, 01 Jan 1970 00:00:00 GMT"
                        + ( domain ? "; domain=" + domain : "")
                        + ( path ? "; path=" + path : "");
    ...
    angular.module('satellizer', [])
        ...
        .service('satellizer.storage', ['satellizer.config', function(config) {

            var namespace = [config.tokenPrefix, config.tokenName].join('_');

            var service = {
                get: function() {
                    storage.get(config.tokenStorage, namespace);
                },
                set: function(token, rememberMe) {
                    if (!config.tokenOptions) {
                        config.tokenOptions = {};
                    }
                    if (rememberMe) {
                        config.tokenOptions.end = Infinity;
                    } else {
                        config.tokenOptions.end = false;
                    }
                    storage.set(config.tokenStorage, namespace, token, config.tokenOptions);
                },
                delete: function() {
                    storage.delete(config.tokenStorage, namespace, config.tokenOptions);
                }
            }
        ...
        .provider('$auth', ['satellizer.config', function(config) {
            Object.defineProperties(this, {
                ...
                tokenStorage: {
                    get: function() { return config.tokenStorage; },
                    set: function(value) { config.tokenStorage = value; }
                },
                tokenOptions: {
                    get: function() { return config.tokenOptions; },
                    set: function(value) { config.tokenOptions = value; }
                },
                unlinkUrl: {
        ...
        .factory('satellizer.shared', [
            ...
            function($q, $window, $location, config, storage) {
                ...
                shared.parseUser = function(response, deferred, rememberMe) {
                    var token = response.data[config.tokenName];
                    storage.set(token, rememberMe);
            ...
            function($q, $http, $location, utils, shared, config) {
                ...
                local.login = function(user) {
                    ...
                    $http.post(config.loginUrl, user)
                        .then(function(response) {
                            shared.parseUser(response, deferred, user.remember_me);
                        })
                        .catch(function(response) {
                            deferred.reject(response);
                        });
            ...
            $httpProvider.interceptors.push(['$q', function($q) {
                ...
                return {
                    request: function(httpConfig) {
                        var token = storage.get(config.tokenStorage, namespace);
                        if (token) {
                    ...
                    responseError: function(response) {
                        if (response.status === 401) {
                            storage.delete(config, namespace)
                        }
